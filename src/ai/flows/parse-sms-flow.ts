...
const ParsedSmsSchema = z.object({
  ...
  currency: z.string().optional().describe("The currency code (e.g., YER, USD, SAR)."),
});
...
3.  استخرج العملة (currency) كرمز (YER, SAR, USD). لاحظ أن "ر.ي" تعني "YER".
4.  استخرج اسم الشخص الآخر في المعاملة (person). قد يأتي الاسم بعد كلمات مثل "من", "لـ", "لحساب", "إلى", أو قد يكون في بداية الجملة مثل "أودع/". في بعض الأحيان يكون الشخص عبارة عن رقم هاتف مثل "جوالي رقم 733969608".
5.  إذا لم تتمكن من تحديد أي من هذه التفاصيل بشكل مؤكد، أو إذا كانت الرسالة لا تبدو كمعاملة مالية، أعد كائن JSON مع {"parsed": false}.
6.  يجب أن يكون الرد الخاص بك بتنسيق JSON حصريًا، مطابقًا للمخطط المحدد.

أمثلة لفهمك:
- "تم تحويل10100لحساب عمار محمد رصيدك14٫83YER" -> {"parsed": true, "type": "debit", "amount": 10100, "person": "عمار محمد", "currency": "YER"}
- "أودع/محمد احمد لحسابك35000 YER" -> {"parsed": true, "type": "credit", "amount": 35000, "person": "محمد احمد", "currency": "YER"}
- "استلمت 6,000.00 من صدام حسن احمد" -> {"parsed": true, "type": "credit", "amount": 6000, "person": "صدام حسن احمد", "currency": "YER"}
- "خصم 53750ر.ي تحويل لمحفظة/بنك ... الى جوالي رقم 733969608" -> {"parsed": true, "type": "debit", "amount": 53750, "person": "جوالي رقم 733969608", "currency": "YER"}
...
export async function parseSmsWithAi(smsBody: string, apiKey: string): Promise<ParsedSms | null> {
    ...
    try {
        ...
        if (!parsedJson.type || !parsedJson.amount || !parsedJson.person || !parsedJson.currency) {
