...
interface InvoiceViewProps {
    ...
    client: Client | null;
    linkedRecords: (CashRecord | UsdtRecord & { recordType?: 'cash' | 'usdt' })[];
}

const getRecordDetails = (record: CashRecord | UsdtRecord & { recordType?: 'cash' | 'usdt' }, type: 'inflow' | 'outflow') => {
    // Check if it's a cash record by looking for a property unique to it, like 'currency'
    if (record.recordType === 'cash') {
        const cashRecord = record as CashRecord;
        return {
            title: `Fiat ${type === 'inflow' ? 'Received' : 'Paid'}`,
            amount: `${cashRecord.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })} ${cashRecord.currency}`,
            icon: Landmark,
            details: [
                { label: type === 'inflow' ? "From" : "To", value: cashRecord.senderName || cashRecord.recipientName, icon: User },
                { label: "Via Account", value: cashRecord.accountName, icon: Landmark },
                { label: "Remittance #", value: cashRecord.notes, icon: Repeat }
            ]
        }
    } else { // It's a UsdtRecord
        const usdtRecord = record as UsdtRecord;
        return {
            title: `USDT ${type === 'inflow' ? 'Received' : 'Paid'}`,
            amount: `${usdtRecord.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })} USDT`,
            icon: Wallet,
            details: [
                { label: type === 'inflow' ? "From Wallet" : "To Wallet", value: usdtRecord.clientWalletAddress, icon: Wallet },
                { label: "System Wallet", value: usdtRecord.accountName, icon: Landmark },
                { label: "Tx Hash", value: usdtRecord.txHash, icon: Hash }
            ]
        }
    }
}
...
export function InvoiceView({ transaction, client, linkedRecords }: InvoiceViewProps) {
    ...
    useEffect(() => {
        ...
        if (transaction.date) {
            try {
                 ...
                 setFormattedTransactionTime('Invalid Date');
            }
        } else {
    ...
    return (
        <div dir="rtl" className="w-full max-w-md mx-auto bg-background text-foreground font-cairo">
