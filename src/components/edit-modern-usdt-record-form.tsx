...
import { updateModernUsdtRecord, type ModernUsdtRecordState, searchClients } from '@/lib/actions';
...
export function EditModernUsdtRecordForm({ record, clients }: { record: ModernUsdtRecord, clients: Client[] }) {
    const { toast } = useToast();
    const actionWithId = updateModernUsdtRecord.bind(null, record.id);
    const [state, formAction] = useActionState<ModernUsdtRecordState, FormData>(actionWithId, undefined);
    ...
    return (
        <form action={formAction}>
             <Card>
                <CardHeader>
                    <CardTitle>Edit USDT Record</CardTitle>
                ...
                <CardContent className="space-y-4">
                     <div className="grid md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                             <Label>Amount</Label>
                             <Input value={`${record.amount.toLocaleString()} USDT`} disabled />
                        </div>
                    </div>
                     <div className="space-y-2">
                        <Label>Client Wallet Address</Label>
                        <Input value={record.clientWalletAddress || 'N/A'} disabled />
                    </div>
                    <div className="space-y-2">
